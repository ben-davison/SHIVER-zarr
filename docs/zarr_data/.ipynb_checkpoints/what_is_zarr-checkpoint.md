...
**Zarr** is a powerful open-source, cloud-native protocol for storing chunked, compressed N-dimensional arrays. It is designed for performance, interoperability and cloud computing or other parallel computing application. It enables high-throughput distributed I/O, allows concurrent read/write from multiple threads or processes. In a world where data is abundant, traditional data formats can become a bottleneck in a workflow - zarr removes that bottleneck.
