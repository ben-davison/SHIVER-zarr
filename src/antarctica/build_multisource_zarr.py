...
OUTPUT_ZARR = os.path.join(OUTPUT_DIR, "Antarctica_multisource_speed_spatial_200.zarr")
CATALOG_FILE = os.path.join(OUTPUT_DIR, "master_epoch_catalog.pkl")
...
def preprocess_measures_ase(file_path, year, master_x, master_y):
    ds_raw = xr.open_dataset(file_path)
    # Explicitly assign xaxis/yaxis as the true spatial coordinates for nx/ny
...
def parse_joughin_tsx_time(file_path):
    ...
    mid_dt = datetime.strptime(match.group(1), "%Y-%m-%d")
    start_dt = mid_dt - timedelta(days=53)
    end_dt = mid_dt + timedelta(days=53)
    return np.datetime64(mid_dt), np.array([np.datetime64(start_dt), np.datetime64(end_dt)])
...
def parse_shift_time(date_str):
    ...
    start_dt = datetime.strptime(start_str, "%Y%m%d")
    # Setting to the end of the day to stay consistent with other datasets
    end_dt = datetime.strptime(end_str, "%Y%m%d").replace(hour=23, minute=59, second=59)
    mid_dt = start_dt + (end_dt - start_dt) / 2
    return np.datetime64(mid_dt), np.array([np.datetime64(start_dt), np.datetime64(end_dt)])

def read_shift_med(filepath):
    """Safely reads the 'med' column (or 6th column fallback) from a SHIFT metadata text file."""
    ...
    try:
        ...
        med_idx = -1
        # Try finding exact match, otherwise fall back to 6th column (index 5)
        if 'med' in headers:
...
def catalog_shift(base_dir):
    ...
    # Safely iterate through all items in the directory
    for d in os.listdir(base_dir):
        # Use regex to strictly match YYYYMMDD_YYYYMMDD
...
def preprocess_shift(file_path, epoch_dir, date_str, master_x, master_y):
    ...
    # 1. Load Speed (Already in m/year) - using Greenland squeeze rule
    ...
    # 2. Extract Error Metadata using Robust parser
    ...
    if np.isnan(error_scalar):
        ...
        # Repmat the single scalar across the entire 2D grid where speed exists
        error_interp = xr.full_like(speed_interp, error_scalar)
        error_interp = error_interp.where(speed_interp.notnull())

    # Explicitly pull .values as in the Greenland code to decouple from xarray internals
...
def build_catalog_and_skeleton():
    ...
    print("Writing skeleton to disk...", flush=True)
    ds_skeleton.to_zarr(OUTPUT_ZARR, compute=False, encoding=encoding, mode='w')
    print("Skeleton initialized successfully!", flush=True)
...
def process_worker(target_source):
    ...
    print(f"Worker for {target_source} completed!", flush=True)
